...
public class BSTree<T extends Comparable<? super T>> implements Iterable {
    ...
    protected class BSTNode {
        ...
        /**
         * Check if two given nodes have the same children
         *
         * @param boolean {@code true} if left and right child are both the same,
         *                {@code false} otherwise
         */
        public boolean checkChildren(BSTNode other) {
            boolean leftFlag1, leftFlag2, rightFlag1, rightFlag2;
            if (left == null) {
                leftFlag1 = false;
            } else {
                leftFlag1 = true;
            ...
            if (leftFlag1 ^ leftFlag2 || rightFlag1 ^ rightFlag2) {
                return true;
            } else {
                if (leftFlag1) {
                    if (left.key.compareTo(other.left.key) == 0) {
                        if (rightFlag1) {
                            if (right.key.compareTo(other.right.key) == 0) {
                                return true;
                            } else {
                                return false;
                            }
    ...
    public boolean equal(BSTree<T> other) {
        BSTNode curNode = getRoot(), otherNode = other.getRoot();
        if (curNode == null && otherNode == null) {
            return true;
        } else if (curNode != null && otherNode != null) {
            if (curNode.key.compareTo(otherNode.key) == 0) {
                return curNode.checkChildren(otherNode);
            } else {
                return false;
            }
        } else {
